...
@Dependent
public class GoogleSuggestConnector implements SuggestConnector {
    ...
    public SuggestConnectorResults suggest(SuggestQuery query) throws ConnectorException {
        UriBuilder requestUri = UriBuilder.fromUri("https://google.com/complete/search");
        requestUri.queryParam("q", query.getQuery());
        requestUri.queryParam("client", "firefox"); // for firefox it returns more results (10 instead of 8 for chrome)
        if (query.getLanguage() != null) {
            requestUri.queryParam("hl", query.getLanguage());
        }
