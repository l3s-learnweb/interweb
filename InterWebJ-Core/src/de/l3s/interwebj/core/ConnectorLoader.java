...
public class ConnectorLoader
{
	...
	Environment.logger.info("loading plugins from directory: [" + pluginDir.getAbsolutePath() + "]");
	...
	{
	    Environment.logger.severe("[" + pluginDirPath + "] is not a directory or doesn't exist");
	}
	return connectors;
    }
	...
	try
	{
	    ...
	    Environment.logger.info("trying load connector from folder: [" + connectorDir.getAbsolutePath() + "]");
	    ...
	    Environment.logger.info("connector jar file: [" + connectorName + "] successfully loaded");
	...
	try
	{
	    ...
	    File connectorJarFile = new File(connectorDir, configuration.getValue("jar-name"));
	    ...
	    {
		e.printStackTrace();
		Environment.logger.severe("No jar file found for connector [" + connectorName + "]. Check configuration file");
		return null;
	    }
	    Environment.logger.info("connector jar file: [" + connectorJarFile.getAbsolutePath() + "] successfully loaded");
	    File libDir = new File(connectorDir, "lib");
	    File[] libJarFiles = getJars(libDir);
	    if(libJarFiles != null)
	    {
		for(File libJarFile : libJarFiles)
		{
		    ...
		    Environment.logger.info("dependency library jar file: [" + libJarFile.getAbsolutePath() + "] successfully loaded");
		}
	    }
