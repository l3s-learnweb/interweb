...
public class AccessControll
{
    ...
    public boolean isAuthorized(InterWebPrincipal principal, String resource, String action)
    {
	...
	{
	    if(resourceConstraint == null && constraint.matches(resource) || resourceConstraint != null && constraint.matches(resource) && constraint.getWeight() > resourceConstraint.getWeight())
	    {
	...
	constraints.add(new ResourceConstraint("/view/admin/(.*)?", InterWebPrincipal.MANAGER_ROLE, 20));
