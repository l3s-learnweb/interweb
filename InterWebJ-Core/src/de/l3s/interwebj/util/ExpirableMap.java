    ...
    implements ConcurrentMap<K, V>
{
	...
	private static final class ExpirationTask<K, V>
	    implements Runnable
	{
	...
	private boolean hasExpired(Expirable<K, V> expirable)
	{
		...
		return !policy.isEternal()
		       && (isPastTime(policy.getTimeToIdle(TimeUnit.NANOSECONDS),
		                      expirable.getAccessTime(TimeUnit.NANOSECONDS),
		                      currentTime) || isPastTime(policy.getTimeToLive(TimeUnit.NANOSECONDS),
		                                                 expirable.getCreationTime(TimeUnit.NANOSECONDS),
		                                                 currentTime));
	}
	...
	private void scheduleTimeToIdle(Expirable<K, V> expirable)
	{
		...
		if (timeToIdle != 0)
		{
			scheduler.schedule(new ExpirationTask<K, V>(this, expirable),
			                   timeToIdle,
